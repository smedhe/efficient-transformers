...
class QEffDynamicCompressedKVRopeLayer:
    ...
    def update_ckv(self, compressed_kv, cache_kwargs):
        ...
        ckv_out = torch.where(
            ...
            torch.zeros_like(ckv_out, dtype=torch.float32),
            ckv_out,
        )
        return ckv_out
    ...
    def update_k_pe(self, k_pe_cache, cache_kwargs):
        ...
        k_pe_out = torch.where(
            invalid_mask.unsqueeze(-1),
            torch.zeros_like(k_pe_out, dtype=torch.float32),
            k_pe_out,
        )
        return k_pe_out
...
class QEffSlidingWindowCache:
    ...
    def from_legacy_cache(
        ...
        cache = cls(
            ...
            max_cache_len=int(past_key_values[sliding_window_pattern - 1][0].shape[2]),
